...
{
    public partial class frmAdmin_Nurse : Form
    {
        SqlConnection con = Database.db_connect();
        int id;
        int x = RealTimeUpdate.x();

        public frmAdmin_Nurse()
        {
            InitializeComponent();
        ...
        {
            using (dlUser dl = new dlUser(0, 0, 0, "Nurse"))
            {
                dl.ShowDialog();
            }
        }
        ...
        private void frmAdmin_Nurse_VisibleChanged(object sender, EventArgs e)
        {
            LoadData(txtSearch.Text);
            Method.lvbtn(lvUser, btnUpdate, btnDelete);
        }
